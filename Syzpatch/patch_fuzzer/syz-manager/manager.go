...
var (
	...
	flagSplice    = flag.Bool("splice", false, "enable splice mutation")
)
...
func (mgr *Manager) SyncRelation() bool {
	mgr.mu.Lock()
	defer mgr.mu.Unlock()
	//log.Logf(1, "sync rpc ok!")
	return true
}
func (mgr *Manager) GetRelation() bool {
	mgr.mu.Lock()
	defer mgr.mu.Unlock()
	return true
}
func (mgr *Manager) SyncRelationBuildingSeed() bool {
	mgr.mu.Lock()
	defer mgr.mu.Unlock()
	//log.Logf(1, "sync rpc ok!")
	return true
}
func (mgr *Manager) GetRelationBuildingSig() bool {
	mgr.mu.Lock()
	defer mgr.mu.Unlock()
	//log.Logf(1, "sync rpc ok!")
	return true
