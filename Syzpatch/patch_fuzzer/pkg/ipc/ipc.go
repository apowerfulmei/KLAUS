...
func (env *Env) parseOutput(p *prog.Prog) (*ProgInfo, error) {
	...
	for i := uint32(0); i < ncmd; i++ {
		...
		inf.Similarity = make([]uint64, 0)
		inf.Cover = make([]uint32, 0)
		inf.Hashvar = make([]uint64, 0)
		inf.HashvarIdx = make([]uint32, 0)
		inf.PreTrace = make([]uint32, 0)
		inf.EnableTrace = make([]uint32, 0)
		inf.PostTrace = make([]uint32, 0)
		tmp_hashvar := uint64(0)
		hashvarflag := 0
		for _, pc := range Cover {
			// if pc>>32 == 0xdeadbeef {
			switch int(pc >> 56) {
			case 0xff:
				inf.Cover = append(inf.Cover, (uint32)(pc))
			case 0:
				inf.Similarity = append(inf.Similarity, (pc))
			case 1:
				if hashvarflag == 0 {
					//inf.Hashvar = append(inf.Hashvar, (uint32)(pc&0x00000000ffffffff))
					tmp_hashvar = (pc & 0x00000000ffffffff) << 32
					inf.HashvarIdx = append(inf.HashvarIdx, (uint32)((pc&0x00ffffffffffffff)>>32))
					hashvarflag = 1
				} else {
					tmp_hashvar = tmp_hashvar | (pc & 0x00000000ffffffff)
					inf.Hashvar = append(inf.Hashvar, tmp_hashvar)
					tmp_hashvar = uint64(0)
					hashvarflag = 0
				}
			...
			case 3:
				//maxid << 32 | idx
				inf.EnableTrace = append(inf.EnableTrace, (uint32)(pc&0xffffffff))
			case 4:
				inf.PreTrace = append(inf.PreTrace, (uint32)(pc&0xffffffff))
			case 5:
				inf.PostTrace = append(inf.PostTrace, (uint32)(pc&0xffffffff))
			}
		}
